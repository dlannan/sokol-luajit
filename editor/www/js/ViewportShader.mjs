import * as Shader from "/js/Shader.mjs";
import * as Mat4 from "/js/Mat4.mjs";
...
function loadMesh() {
	const mesh = { quad: "/models/plane.obj" };
...
export function updateImage(textureUniformName, image) {
	...
	const context3D = viewport3D.canvas.getContext("webgl");
	const context2D = viewport2D.canvas.getContext("webgl");
...
function setup3D() {
	const gl = viewport3D.canvas.getContext("webgl");
console.log(gl);
...
function setup2D() {
	const gl = viewport2D.canvas.getContext("webgl");
