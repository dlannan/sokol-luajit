...
local function defaulthandler( self, delta )
	...
	local campos 	 = vmath.matrix4_from_quat(camrot) * hmm.HMM_Vec4(0, 0, self.distance, 0)

	self.prevtpos 	 = hmm.HMM_Vec3(self.tpos)
	local ospos 	 = go.get_position(self.target)
	self.tpos 		 = hmm.HMM_Vec3(ospos.x, ospos.y, ospos.z)

	local tgtrot 	 = go.get_rotation(self.target)
	local osfwd      = tf.QuatToDir( tgtrot )
	self.trot 		 = tf.DirToQuat(hmm.HMM_Vec3(osfwd.x, osfwd.y, osfwd.z))
	
	self.tspeed 	 = 0.0
	self.teuler		 = tf.ToEulerAngles(self.trot)
	self.tvelocity 	 = hmm.HMM_Vec3(0,0,0)

	self.throttle	 = math.floor(self.tspeed/10.0 * 10)
	
	if(self.use_target == nil and self.tPos) then self.tpos = self.tPos end
	self.pos = self.tpos + hmm.HMM_Vec3(campos.x, campos.y, campos.z)
	self.rot = camrotinv
...
cameraorbit.init = function( cameraobj, target, distance, handler )
	...
	local tvec = hmm.HMM_Vec3(0,0,0) -- go.get_position(target)
	newcam.tpos			= hmm.HMM_Vec3(tvec.X, tvec.Y, tvec.Z)
