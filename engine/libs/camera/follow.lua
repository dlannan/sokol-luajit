...
local function LookAt(sourcePoint, destPoint)
	...
	local dot = vmath.dot(hmm.HMM_Vec3(0.0, 0.0, 1.0), forwardVector)
	...
	local rotAxis = vmath.cross(hmm.HMM_Vec3(0.0,0.0,1.0), forwardVector)
...
local function defaulthandler( self, delta )
	...
	local sloppyrot  = hmm.HMM_Slerp(self.rot, delta * self.sloppiness, self.trot)
	local rotmat 	 = hmm.HMM_QuaternionToMat4(sloppyrot)
	local mdir 	 	 = rotmat * hmm.HMM_Vec4(0, self.cameraheight, self.distance, 0)
