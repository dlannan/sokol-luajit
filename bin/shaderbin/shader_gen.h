...
/*
    ...
    Cmdline:
        sokol-shdc -i ./samples/offscreen-sapp.glsl -o F:\dev\luajit\Thunc\sokol-luajit\bin/shaderbin/shader_gen.h -l glsl410 -f sokol
...
static inline const sg_shader_desc* default_shader_desc(sg_backend backend) {
    if (backend == SG_BACKEND_GLCORE) {
        ...
        if (!valid) {
            ...
            desc.vs.source = (const char*)vs_default_source_glsl410;
            desc.vs.entry = "main";
            ...
            desc.fs.source = (const char*)fs_default_source_glsl410;
            desc.fs.entry = "main";
...
static inline const sg_shader_desc* offscreen_shader_desc(sg_backend backend) {
    if (backend == SG_BACKEND_GLCORE) {
        ...
        if (!valid) {
            ...
            desc.attrs[0].name = "position";
            desc.attrs[1].name = "normal";
            desc.vs.source = (const char*)vs_offscreen_source_glsl410;
            desc.vs.entry = "main";
            desc.vs.uniform_blocks[0].size = 64;
            desc.vs.uniform_blocks[0].layout = SG_UNIFORMLAYOUT_STD140;
            desc.vs.uniform_blocks[0].uniforms[0].name = "vs_params";
            desc.vs.uniform_blocks[0].uniforms[0].type = SG_UNIFORMTYPE_FLOAT4;
            desc.vs.uniform_blocks[0].uniforms[0].array_count = 4;
            desc.fs.source = (const char*)fs_offscreen_source_glsl410;
            desc.fs.entry = "main";
