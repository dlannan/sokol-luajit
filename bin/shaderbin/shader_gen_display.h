...
/*
    ...
    Cmdline:
        sokol-shdc -i ./projects/examples/samples/shadows-sapp.glsl -o F:\dev\luajit\repos\sokol-luajit\bin\shaderbin\shader_gen_display.h -l glsl410 -f sokol
...
#error "Please include sokol_gfx.h before shader_gen_display.h"
#endif
...
static inline const sg_shader_desc* dbg_shader_desc(sg_backend backend) {
    if (backend == SG_BACKEND_GLCORE) {
        ...
        if (!valid) {
            valid = true;
            desc.vertex_func.source = (const char*)vs_dbg_source_glsl410;
            desc.vertex_func.entry = "main";
            desc.fragment_func.source = (const char*)fs_dbg_source_glsl410;
...
static inline const sg_shader_desc* display_shader_desc(sg_backend backend) {
    if (backend == SG_BACKEND_GLCORE) {
        ...
        if (!valid) {
            valid = true;
            desc.vertex_func.source = (const char*)vs_display_source_glsl410;
            desc.vertex_func.entry = "main";
            desc.fragment_func.source = (const char*)fs_display_source_glsl410;
            desc.fragment_func.entry = "main";
            desc.attrs[0].glsl_name = "pos";
